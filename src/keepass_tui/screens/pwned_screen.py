...
def screen_check_all(stdscr, entries) -> None:
    ...
    for i, entry in enumerate(has_passwords):
        _draw_progress(
            stdscr, f"Проверяю {i + 1}/{total}: {trunc(entry.title or '', 40)}..."
        )
...
def _draw_progress(stdscr, text: str) -> None:
    ...
    draw_box(stdscr, 0, 0, h, w, "Проверка утечек")
    safe_addstr(
        stdscr, h // 2, 3, trunc(text, w - 6), curses.color_pair(C_WARN) | curses.A_BOLD
    )
    stdscr.refresh()
...
def _show_mass_result(stdscr, results: list) -> None:
    ...
    while True:
        ...
        for r in pwned_list[:list_h]:
            ...
            safe_addstr(
                stdscr, row, 3, f"✗ {title}", curses.color_pair(C_WARN) | curses.A_BOLD
            )
            safe_addstr(
                stdscr, row, w // 2, f"вхождений: {count}", curses.color_pair(C_WARN)
            )
            row += 1
        ...
        for r in error_list:
            if row >= h - 4:
                break
            title = trunc(r["entry"].title or "", w // 2 - 4)
            safe_addstr(stdscr, row, 3, f"⚠ {title}", curses.color_pair(C_WARN))
            safe_addstr(
                stdscr,
                row,
                w // 2,
