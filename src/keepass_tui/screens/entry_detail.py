...
def screen_entry_detail(stdscr, entry, kp=None, db_path=None) -> str | None:
    ...
    while True:
        ...
        for label, value in fields:
            ...
            safe_addstr(
                stdscr, row, 3, f"{label}:", curses.color_pair(C_TITLE) | curses.A_BOLD
            )
            ...
            if label == "Пароль":
                ...
                safe_addstr(
                    stdscr, row, 14, trunc(display, w - 16), curses.color_pair(C_VALUE)
                )
            else:
                safe_addstr(
                    stdscr, row, 14, trunc(value, w - 16), curses.color_pair(C_DIM)
                )
            row += 2
