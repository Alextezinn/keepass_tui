...
def screen_change_password(stdscr, entry, kp, db_path) -> None:
    ...
    if not ip:
        ...
        safe_addstr(stdscr, h // 2 + 1, 3, "Нажмите любую клавишу...",
...
def _confirm_change(stdscr, ip, username, current_password) -> bool:
    ...
    safe_addstr(stdscr, 2,  3, "Будет выполнена смена пароля на сервере:",
    ...
    safe_addstr(stdscr, 8,  20, "●" * min(len(current_password), 20),
    ...
    render_hint(stdscr, h - 3, w, [
        ("Enter", "подтвердить и сменить"), ("q", "отмена")
    ])
    stdscr.refresh()
...
def _confirm_mass_change(stdscr, count: int) -> bool:
    ...
    h, w = stdscr.getmaxyx()
    draw_box(stdscr, 0, 0, h, w, "Массовая смена паролей")
    safe_addstr(stdscr, h // 2 - 2, 3,
        f"Будет выполнена смена пароля для {count} записей.",
        curses.color_pair(C_WARN) | curses.A_BOLD)
    safe_addstr(stdscr, h // 2, 3, "Это необратимая операция. Продолжить?",
                curses.color_pair(C_DIM))
...
def _show_change_result(stdscr, entry, kp, ip, new_password, error) -> None:
    ...
    if error:
        safe_addstr(stdscr, h // 2 - 2, 3, "⚠  Ошибка при смене пароля:",
                    curses.color_pair(C_WARN) | curses.A_BOLD)
        safe_addstr(stdscr, h // 2, 3, trunc(error, w - 6),
                    curses.color_pair(C_WARN))
        ...
        if save_ok:
            ...
            safe_addstr(stdscr, h // 2, 3,
                ...
                curses.color_pair(C_WARN))
            safe_addstr(stdscr, h // 2 + 2, 3,
                f"Новый пароль: {new_password}", curses.color_pair(C_VALUE))
...
def _show_mass_result(stdscr, success: int, failed: int, results: list) -> None:
    ...
    safe_addstr(stdscr, 2, 3, f"Успешно: {success} | Ошибок: {failed}",
